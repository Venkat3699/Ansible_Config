...
resource "aws_route" "peering_to_ansible_vpc" {
  ...
  destination_cidr_block    = "172.31.0.0/16"  # Replace with the actual CIDR block of the peer VPC
...
resource "aws_route" "peering_from_ansible_vpc" {
  ...
  destination_cidr_block    = "10.0.0.0/16"  # Replace with the actual CIDR block of your VPC
  vpc_peering_connection_id  = aws_vpc_peering_connection.ansible_vpc_peering.id
}